...
pub fn get_row_from_settings_by_name(
	conn: &Connection,
	name: &str,
) -> Result<DBSetting, RusqliteError> {
	let result = conn.query_row::<DBSetting, _, _>(
...
impl DBSetting {
	pub fn _get_from_db_by(name: &str) -> Result<DBSetting, RusqliteError> {
